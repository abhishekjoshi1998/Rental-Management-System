...
import LeaseListItem from '../../components/Leases/LeaseListItem';
  ...
  return (
    <div>
      ...
      {(user?.role === 'landlord' || user?.role === 'property_manager') && (
        <Link to="/leases/create" className="button-like">Create New Lease</Link>
      )}
      {leases.length === 0 ? (
        <p>No leases found.</p>
      ) : (
        leases.map(lease => (
          <LeaseListItem key={lease._id} lease={lease} />
        ))
      )}
    </div>
  );
